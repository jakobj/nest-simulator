...
#pragma omp single
  {
          ...
          ++cit )
    {
      ...
      buffer_pos_of_source_gid_syn_id.insert( std::make_pair(
        pack_source_gid_and_syn_id( cit->first, cit->second ), recv_buffer_position_by_rank[ source_rank ] ) );
  ...
  TargetData& next_target_data )
{
  ...
  while ( true )
  {
    ...
    source_rank = kernel().mpi_manager.get_process_id_of_gid( const_current_source.get_gid() );
    ...
    else
    {
      ...
      if ( current_source.is_primary() )
      {
        ...
        if ( current_source.is_compressed() )
        {
          ...
          auto it_idx = compressed_spike_data_map_[ current_position.tid ][ current_position.syn_id ].find( current_source.get_gid() );
  ...
  for ( synindex syn_id = 0; syn_id < sources_[ tid ].size(); ++syn_id )
  {
    ...
    while ( lcid < sources_[ tid ][ syn_id ].size() )
    {
      ...
      if ( ( ( lcid + 1 ) < sources_[ tid ][ syn_id ].size() )
           and ( sources_[ tid ][ syn_id ][ lcid + 1 ].get_gid() == sources_[ tid ][ syn_id ][ lcid ].get_gid() ) )
      {
        compressible_sources_[ tid ][ syn_id ].insert( std::make_pair( sources_[ tid ][ syn_id ][ lcid ].get_gid(),
                                                                       SpikeData( tid, syn_id, lcid, 0 ) ) );
      }

      // find next source with different gid (assumes sorted sources)
      const index old_source_gid = sources_[ tid ][ syn_id ][ lcid ].get_gid();
      ++lcid;
      while ( ( lcid < sources_[ tid ][ syn_id ].size() )
              and ( sources_[ tid ][ syn_id ][ lcid ].get_gid() == old_source_gid ) )
      {
        ++lcid;
      }
    }
