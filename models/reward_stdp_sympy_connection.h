...
#ifndef REWARD_STDP_SYMPY_CONNECTION_H
#define REWARD_STDP_SYMPY_CONNECTION_H
...
// make sure NEST is compiled without MPI
#ifdef HAVE_MPI
  typedef nest::StaticAssert< false >::success have_mpi;
#endif
...
class RewardSTDPSympyConnectionCommonProperties : public CommonSynapseProperties
{
public:
  RewardSTDPSympyConnectionCommonProperties();
...
class RewardSTDPSympyConnection : public Connection< targetidentifierT >
{

public:
  typedef RewardSTDPSympyConnectionCommonProperties CommonPropertiesType;
   ...
   */
  RewardSTDPSympyConnection( const RewardSTDPSympyConnection& );
   ...
   */
  void send( Event& e, thread t, const CommonPropertiesType& cp );
...
private:
  double
  facilitate_( double w, double kplus, const CommonPropertiesType& cp )
  {
    ...
    const double delta_w = expr_facilitate_.eval( { weight_ / Wmax_, kplus, get_reward_( cp ) } );
    const double norm_w = ( w / Wmax_ ) + lambda_ * delta_w;
    return norm_w < 1.0 ? norm_w * Wmax_ : Wmax_;
  }

  double
  depress_( double w, double kminus, const CommonPropertiesType& cp )
  {
    // double norm_w = ( w / Wmax_ )
    //   - ( alpha_ * lambda_ * std::pow( w / Wmax_, mu_minus_ ) * kminus );
    const double delta_w = expr_depress_.eval( { weight_ / Wmax_, kminus, get_reward_( cp ) } );
    const double norm_w = ( w / Wmax_ ) - alpha_ * lambda_ * delta_w;
    return norm_w > 0.0 ? norm_w * Wmax_ : 0.0;
  }

  void
  set_reward_node_( const long vtgid );

  double
  get_reward_( const CommonPropertiesType& cp ) const
  {
    return cp.vt_->get_V_m_();
  }
...
template < typename targetidentifierT >
inline void
RewardSTDPSympyConnection< targetidentifierT >::send( Event& e,
  thread t,
  const CommonPropertiesType& cp )
{
  ...
  while ( start != finish )
  {
    ...
    weight_ = facilitate_( weight_, Kplus_ * std::exp( minus_dt / tau_plus_), cp );
  }
...
template < typename targetidentifierT >
RewardSTDPSympyConnection< targetidentifierT >::RewardSTDPSympyConnection()
  : ConnectionBase()
  , weight_( 1.0 )
  , tau_plus_( 20.0 )
  , lambda_( 0.01 )
...
template < typename targetidentifierT >
RewardSTDPSympyConnection< targetidentifierT >::RewardSTDPSympyConnection(
  const RewardSTDPSympyConnection< targetidentifierT >& rhs )
  : ConnectionBase( rhs )
  , weight_( rhs.weight_ )
...
template < typename targetidentifierT >
void
RewardSTDPSympyConnection< targetidentifierT >::get_status( DictionaryDatum& d ) const
{
...
template < typename targetidentifierT >
void
RewardSTDPSympyConnection< targetidentifierT >::set_status( const DictionaryDatum& d,
  ConnectorModel& cm )
{
...
#endif // of #ifndef REWARD_STDP_SYMPY_CONNECTION_H
