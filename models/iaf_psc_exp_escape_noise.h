...
#ifndef IAF_PSC_EXP_ESCAPE_NOISE_H
#define IAF_PSC_EXP_ESCAPE_NOISE_H
...
{
/* BeginDocumentation
   Name: iaf_psc_exp_escape_noise - Leaky integrate-and-fire neuron model with exponential
                       PSCs and stochastic spiking

   Pfister, J. P., Toyoizumi, T., Barber, D., & Gerstner,
   W. (2006). Optimal spike-timing-dependent plasticity for precise
   action potential firing in supervised learning. Neural computation,
   18(6), 1318-1348.

   Description:
   iaf_psc_exp_escape_noisep is an implementation of a leaky integrate-and-fire model
   ...
   optimization levels. A future version of iaf_psc_exp_escape_noise will probably
...
Remarks:
   ...
   iaf_psc_exp_escape_noise can handle current input in two ways: Current input
   ...
   SeeAlso: iaf_psc_exp_escape_noise_ps
 ...
 */
class iaf_psc_exp_escape_noise : public Archiving_Node
{

public:
  iaf_psc_exp_escape_noise();
  iaf_psc_exp_escape_noise( const iaf_psc_exp_escape_noise& );
...
private:
  ...
  friend class RecordablesMap< iaf_psc_exp_escape_noise >;
  friend class UniversalDataLogger< iaf_psc_exp_escape_noise >;
  ...
  struct Buffers_
  {
    Buffers_( iaf_psc_exp_escape_noise& );
    Buffers_( const Buffers_&, iaf_psc_exp_escape_noise& );
    ...
    UniversalDataLogger< iaf_psc_exp_escape_noise > logger_;
  };
  ...
  /**
   * @defgroup iaf_psc_exp_escape_noise_data
...
inline port
nest::iaf_psc_exp_escape_noise::send_test_event( Node& target,
  rport receptor_type,
  synindex,
  bool )
{
...
inline port
iaf_psc_exp_escape_noise::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
iaf_psc_exp_escape_noise::handles_test_event( CurrentEvent&, rport receptor_type )
{
  if ( receptor_type == 0 )
  {
    return 0;
...
inline port
iaf_psc_exp_escape_noise::handles_test_event( DataLoggingRequest& dlr, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline void
iaf_psc_exp_escape_noise::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d, P_ );
...
inline void
iaf_psc_exp_escape_noise::set_status( const DictionaryDatum& d )
{
...
inline double
iaf_psc_exp_escape_noise::phi_() const
{
  return rho_ * std::exp( 1 / delta_ * (S_.V_m_ - P_.Theta_ ) );
}

} // namespace

#endif // IAF_PSC_EXP_ESCAPE_NOISE_H
