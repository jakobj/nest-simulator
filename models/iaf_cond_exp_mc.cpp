...
*/
std::vector< Name > nest::iaf_cond_exp_mc::comp_names_( NCOMP );
...
// DictionaryDatum nest::iaf_cond_exp_mc::receptor_dict_ = new Dictionary();

/* ----------------------------------------------------------------
 * Recordables map
 * ---------------------------------------------------------------- */

nest::RecordablesMap< nest::iaf_cond_exp_mc >
  nest::iaf_cond_exp_mc::recordablesMap_;
...
RecordablesMap< iaf_cond_exp_mc >::create()
{
  ...
  insert_( names::t_ref_remaining, &iaf_cond_exp_mc::get_r_ );
}
}
...
extern "C" int
nest::iaf_cond_exp_mc_dynamics( double,
  const double y[],
  double f[],
  void* pnode )
{
  // some shorthands
  typedef nest::iaf_cond_exp_mc N;
  typedef nest::iaf_cond_exp_mc::State_ S;

  // get access to node so we can work almost as in a member function
  assert( pnode );
  const nest::iaf_cond_exp_mc& node =
    *( reinterpret_cast< nest::iaf_cond_exp_mc* >( pnode ) );
  ...
  for ( size_t n = 0; n < N::NCOMP; ++n )
  {
    ...
    // const double I_conn =  // TODO: couple all compartments
    //   ( n == N::SOMA ? node.P_.g_conn[ 1 ] * ( V - y[ S::idx( 1, S::V_M ) ] ) : 0. );
    ...
    f[ S::idx( n, S::G_EXC ) ] = - y[ S::idx( n, S::G_EXC ) ] / node.P_.tau_synE[ n ];
    ...
    f[ S::idx( n, S::G_INH ) ] = - y[ S::idx( n, S::G_INH ) ] / node.P_.tau_synI[ n ];
  }
...
nest::iaf_cond_exp_mc::Parameters_::Parameters_()
  : V_th( -55.0 )    // mV
  , V_reset( -60.0 ) // mV
  , t_ref( 2.0 )     // ms
{
  ...
  E_in[ DENONE ] = -85.0;   // mV
  E_L[ DENONE ] = -70.0;    // mV
  tau_synE[ DENONE ] = 0.5; // ms
  tau_synI[ DENONE ] = 2.0; // ms
  I_e[ DENONE ] = 0.0;      // pA
  ...
  E_in[ DENTWO ] = -85.0;   // mV
  E_L[ DENTWO ] = -70.0;    // mV
  tau_synE[ DENTWO ] = 0.5; // ms
  tau_synI[ DENTWO ] = 2.0; // ms
  I_e[ DENTWO ] = 0.0;      // pA
}

nest::iaf_cond_exp_mc::Parameters_::Parameters_( const Parameters_& p )
  : V_th( p.V_th )
  , V_reset( p.V_reset )
  , t_ref( p.t_ref )
{
  // copy C-arrays
  for ( size_t n = 0; n < 2 * NCOMP - 1; ++n )
  {
    g_conn[ n ] = p.g_conn[ n ];
  }
...
nest::iaf_cond_exp_mc::Parameters_& nest::iaf_cond_exp_mc::Parameters_::
...
nest::iaf_cond_exp_mc::State_::State_( const Parameters_& p )
  : r_( 0 )
{
...
nest::iaf_cond_exp_mc::State_::State_( const State_& s )
  : r_( s.r_ )
{
  for ( size_t i = 0; i < STATE_VEC_SIZE; ++i )
  {
...
nest::iaf_cond_exp_mc::State_& nest::iaf_cond_exp_mc::State_::operator=(
  const State_& s )
{
...
nest::iaf_cond_exp_mc::Buffers_::Buffers_( iaf_cond_exp_mc& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
nest::iaf_cond_exp_mc::Buffers_::Buffers_( const Buffers_&,
  iaf_cond_exp_mc& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
...
void
nest::iaf_cond_exp_mc::Parameters_::get( DictionaryDatum& d ) const
{
...
void
nest::iaf_cond_exp_mc::Parameters_::set( const DictionaryDatum& d )
{
...
void
nest::iaf_cond_exp_mc::State_::get( DictionaryDatum& d ) const
{
...
void
nest::iaf_cond_exp_mc::State_::set( const DictionaryDatum& d,
  const Parameters_& )
{
  // extract from sub-dictionaries
  for ( size_t n = 0; n < NCOMP; ++n )
...
nest::iaf_cond_exp_mc::iaf_cond_exp_mc()
  : Archiving_Node()
  , P_()
  , S_( P_ )
  , B_( *this )
{
  ...
  comp_names_[ DENONE ] = Name( "denone" );
  comp_names_[ DENTWO ] = Name( "dentwo" );
}

nest::iaf_cond_exp_mc::iaf_cond_exp_mc( const iaf_cond_exp_mc& n )
  : Archiving_Node( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
nest::iaf_cond_exp_mc::init_state_( const Node& proto )
{
  const iaf_cond_exp_mc& pr = downcast< iaf_cond_exp_mc >( proto );
  S_ = pr.S_;
}

void
nest::iaf_cond_exp_mc::init_buffers_()
{
  ...
  B_.sys_.function = iaf_cond_exp_mc_dynamics;
...
void
nest::iaf_cond_exp_mc::calibrate()
{
...
void
nest::iaf_cond_exp_mc::update( Time const& origin,
  const long from,
  const long to )
{
  ...
  for ( long lag = from; lag < to; ++lag )
  {
    ...
    for ( size_t n = 0; n < NCOMP; ++n )
    {
      S_.y_[ n * State_::STATE_VEC_COMPS + State_::G_EXC ] +=
        B_.spikes_[ 2 * n ].get_value( lag );
      S_.y_[ n * State_::STATE_VEC_COMPS + State_::G_INH ] +=
        B_.spikes_[ 2 * n + 1 ].get_value( lag );
    }
...
void
nest::iaf_cond_exp_mc::handle( SpikeEvent& e )
{
...
void
nest::iaf_cond_exp_mc::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void
nest::iaf_cond_exp_mc::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
