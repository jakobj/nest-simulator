...
void
nest::music_rate_in_proxy::Parameters_::set( const DictionaryDatum& d,
  State_& s )
{
...
void
nest::music_rate_in_proxy::State_::set( const DictionaryDatum&,
  const Parameters_& )
{
}
...
nest::music_rate_in_proxy::calibrate()
{
  ...
  if ( !S_.registered_ )
  {
    kernel().music_manager.register_music_rate_in_proxy(
...
void
nest::music_rate_in_proxy::update( Time const& origin,
  const long from,
  const long to )
{
}
