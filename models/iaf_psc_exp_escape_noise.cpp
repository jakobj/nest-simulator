...
nest::RecordablesMap< nest::iaf_psc_exp_escape_noise > nest::iaf_psc_exp_escape_noise::recordablesMap_;
...
template <>
void
RecordablesMap< iaf_psc_exp_escape_noise >::create()
{
  // use standard names whereever you can for consistency!
  insert_( names::V_m, &iaf_psc_exp_escape_noise::get_V_m_ );
  insert_( names::weighted_spikes_ex, &iaf_psc_exp_escape_noise::get_weighted_spikes_ex_ );
  insert_( names::weighted_spikes_in, &iaf_psc_exp_escape_noise::get_weighted_spikes_in_ );
  insert_( names::I_syn_ex, &iaf_psc_exp_escape_noise::get_I_syn_ex_ );
  insert_( names::I_syn_in, &iaf_psc_exp_escape_noise::get_I_syn_in_ );
}
}
...
nest::iaf_psc_exp_escape_noise::Parameters_::Parameters_()
  : Tau_( 10.0 )             // in ms
  , C_( 250.0 )              // in pF
  , t_ref_( 2.0 )            // in ms
  , E_L_( -70.0 )            // in mV
...
nest::iaf_psc_exp_escape_noise::State_::State_()
  : i_0_( 0.0 )
  , i_syn_ex_( 0.0 )
  , i_syn_in_( 0.0 )
  , V_m_( 0.0 )
...
void
nest::iaf_psc_exp_escape_noise::Parameters_::get( DictionaryDatum& d ) const
{
...
double
nest::iaf_psc_exp_escape_noise::Parameters_::set( const DictionaryDatum& d )
{
...
void
nest::iaf_psc_exp_escape_noise::State_::get( DictionaryDatum& d, const Parameters_& p ) const
{
  def< double >( d, names::V_m, V_m_ + p.E_L_ ); // Membrane potential
}

void
nest::iaf_psc_exp_escape_noise::State_::set( const DictionaryDatum& d,
  const Parameters_& p,
  double delta_EL )
{
...
nest::iaf_psc_exp_escape_noise::Buffers_::Buffers_( iaf_psc_exp_escape_noise& n )
  : logger_( n )
{
}

nest::iaf_psc_exp_escape_noise::Buffers_::Buffers_( const Buffers_&, iaf_psc_exp_escape_noise& n )
  : logger_( n )
{
}
...
nest::iaf_psc_exp_escape_noise::iaf_psc_exp_escape_noise()
  : Archiving_Node()
  , P_()
  , S_()
  , B_( *this )
...
nest::iaf_psc_exp_escape_noise::iaf_psc_exp_escape_noise( const iaf_psc_exp_escape_noise& n )
  : Archiving_Node( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
nest::iaf_psc_exp_escape_noise::init_state_( const Node& proto )
{
  const iaf_psc_exp_escape_noise& pr = downcast< iaf_psc_exp_escape_noise >( proto );
  S_ = pr.S_;
}

void
nest::iaf_psc_exp_escape_noise::init_buffers_()
{
...
void
nest::iaf_psc_exp_escape_noise::calibrate()
{
  ...
  // a double in ms. The grid based iaf_psc_exp_escape_noise can only handle refractory
...
void
nest::iaf_psc_exp_escape_noise::update( const Time& origin, const long from, const long to )
{
  assert(
...
void
nest::iaf_psc_exp_escape_noise::handle( SpikeEvent& e )
{
...
void
nest::iaf_psc_exp_escape_noise::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void
nest::iaf_psc_exp_escape_noise::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}

void
nest::iaf_psc_exp_escape_noise::handle( TimeDrivenSpikeEvent& e )
{
  assert( e.get_delay() > 0 );
  assert( time_driven_ );
